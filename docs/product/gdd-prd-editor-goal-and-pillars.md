...
**The document is the product: humans and agents work on the same writable content;
select any span to open its own document — a discussion you can dive into recursively
and collapse back into the head, without ever leaving the surface.**

For agentic work, the shared typed document/meaning graph is canonical. Workspace
contents are addressed through authorized projections rather than copied wholesale
into a transcript; code changes are materialized and verified by the workspace owner.
The ACP-client-first integration sequence is defined in
[ADR 0012](../decisions/0012-writable-agent-doc-and-acp-client-first.md).
...
- **What:** Free, fluid editing. Optional structure (headings, lists) supports formats
  without getting in the way. The shared typed document/meaning graph is the source
  of truth; the conversation, ACP session and harness activity are projections of
  edits and work on it. Human and agent changes are attributed and versioned.
...
- **What:** The app depends on an **inference interface**, not a provider (implementations
  in packages, e.g. `xsoulspace_inference`). The agent participates inside documents:
  it edits or proposes changes **at the cursor, selected part or authorized workspace
  projection**. Agent output lands as attributed document operations with revision and
  diff history, not only as a chat response. Promoting a scoped proposal into the
  materialized code workspace remains an explicit, permission-controlled action.
...
- Remote real-time collaboration is not required for the first local
  human-plus-ACP-agent slice; same-world multi-device collaboration follows
  the mesh gates in ADRs 0005/0008.
- Provider-controlled cloud storage of document/workspace contents. Local-first
  peer sync is the direction; an Internet/VPS relay is a later opt-in transport
  gate with its own privacy and encryption acceptance, not a product data store.
