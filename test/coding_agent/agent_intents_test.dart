...
/// continuation. The host CLASSIFIES prompts while an escalation is
/// pending: the `agent_task_guide` marker (`continue with guidance: …`)
/// is STRIPPED and the guidance rides the repair hint under the host-
/// declared prefix `Operator guidance (escalation round N):` — so the
/// mover keys on that delivered channel, never on the composer sentence.
...
final class GuidedFixMover implements GenerationHandler {
  ...
  Future<ActorGenerateResponse> generate(
    ...
    final guided = request.prompt.contains(
...
void main() {
  ...
  testWidgets(
    ...
    (final tester) async {
      ...
      final result = await _entry('agent_doc_bind').invokeDirect({
      ...
      await pumpUntil(
        tester,
        () => controller.config.checkCommand?.join(' ') == 'dart run main.dart',
      );

      // Absolute paths only — a relative path is refused honestly.
      final bad = await _entry(
        'agent_doc_bind',
  ...
  testWidgets(
    ...
    (final tester) async {
      ...
      final guide = await _entry('agent_task_guide').invokeDirect({
        'guidance':
            'the file throws in main — replace the throw with a '
  ...
  testWidgets(
    ...
    (final tester) async {
      ...
      expect(
        ...
        'meaning',
        reason:
            'AFM works through the meaning tree — never command tools '
      ...
      expect(
        ...
        isFalse,
        reason:
            'the conventional profile stays available for scripted '
