...
- Status: Accepted (direction; remote implementation gates are staged in
  [PLAN.md](../PLAN.md), 5.G1–5.G5)
- Date: 2026-09-06
- North Star impact: `clarifies` (of ADR 0003's workspace-owner authority
  law and the multiplayer sketch; sequencing clarified by ADR 0012)
...
This mapping describes how persisted activity and document fields are
represented; it does not make the transcript the canonical work surface.
Per [ADR 0012](0012-writable-agent-doc-and-acp-client-first.md), durable
document/meaning operations are shared work state, while transcript/activity
views project from those operations and typed execution facts.
...
- The device that owns the workspace is the **simulation authority**:
  agents execute there; host/world snapshots stay device-local. The shared
  document/meaning operation graph is replicated separately.
- Every other paired peer holds a **replica** with input rights: text
  edits, task claims, permission answers, proposals — delivered as attributed
  ops, subject to document access and permission policy.
- One world per workspace and one filesystem materializer/oracle are
  preserved. The shared CRDT document graph accepts attributed operations
  from authorized participants; the workspace owner alone materializes code
  changes and runs the toolchain oracle (ADR 0008). Transport membership
  alone grants neither document access nor execution authority. Sessions are
  projections (see ADR 0006).
...
Remote participation gates live in [PLAN.md](../PLAN.md) under staged work
(5.G1–5.G5). The headline gate
