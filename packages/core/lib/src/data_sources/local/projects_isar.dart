...
final class ProjectsLocalDataSourceIsarImpl implements ProjectsLocalDataSource {
  ProjectsLocalDataSourceIsarImpl({required this.isarDb});
  ...
  Future<PaginatedPageResponseModel<ProjectModel>> getPaginated({
    ...
    var basicQuery = isarDb.projects.where().anyOf(
      types,
      (final q, final type) => q.typeEqualTo(type.name),
    );
    if (data != null) {
      if (data.search.isNotEmpty) {
        basicQuery = basicQuery.and().jsonContentMatches(
          '*${data.search}*',
          caseSensitive: false,
        );
      }
  ...
  Future<List<ProjectModel>> getAll({final RequestProjectsDto? dto}) async {
    ...
    if (dto != null) {
      if (dto.search.isNotEmpty) {
        basicQuery = basicQuery.and().jsonContentContains(
          '*${dto.search}*',
          caseSensitive: false,
        );
      }
    ...
    return items.map((final e) => ProjectModel.fromJson(e.jsonMap)).toList();
  }

  @override
  Future<ProjectModel?> getById({required final ProjectModelId id}) async {
    final project = isarDb.projects
        .where()
        .modelIdStrEqualTo(id.value)
        .findFirst();
  ...
  Future<List<ProjectModel>> getByIds({
    ...
    final projects = isarDb.projects
        ...
        .findAll();
    return projects.map((final e) => ProjectModel.fromJson(e.jsonMap)).toList();
  }
}
