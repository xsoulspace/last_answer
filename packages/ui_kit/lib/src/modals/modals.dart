...
class Modals {
  ...
  Future<T?> showConstrainedDialog<T>({required final WidgetBuilder builder}) =>
  ...
  Future<T?> showActionDialog<T>({
    ...
    builder: (final context) => AlertDialog.adaptive(
      actions: actionsBuilder(
        context,
      ).map((final e) => DialogActionButton(item: e)).toList(),
      title: title,
      content: content,
    ),
  );
...
class DialogActionButton extends StatelessWidget {
  const DialogActionButton({required this.item, super.key});
  ...
  Widget build(final BuildContext context) {
    ...
    if (PlatformInfo.isCupertino) {
      ...
      if (item.isDestructiveAction) {
        return OutlinedButton(onPressed: onPressed, child: Text(item.title));
      }
