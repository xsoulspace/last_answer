...
Here "ACP client" refers to the existing generic chat-document ACP/inference
path; it is not evidence that the agent-document surface can launch an
external ACP coding agent against the writable shared workspace. That
outbound integration is a separate first-slice gate under
[ADR 0012](0012-writable-agent-doc-and-acp-client-first.md) and
[PLAN.md](../PLAN.md). External ACP clients operating Last Answer are a later,
reverse-direction gate.
