...
3. **State split is law**: the shared typed document/meaning graph and its
   durable operations are collaborative doc data (synced, archived,
   shareable). Conversation, task board, roster, settings and permission
   views project from document operations and typed execution facts.
   Host snapshots, derived cuts and runtime state stay **device-local**
   (`.dart_tool/`); they are re-derived or restored from their local snapshot
   store, never from a transcript.
...
5. **Agent-friendly = projections of the same typed state.** The UI renders
   the writable document and its activity; agent-facing surfaces project the
   same state as canonical rows (AE `ProblemRowWire`-compatible) and, later,
   registered IntentCall intents (`agent.doc.create`, `agent.task.delegate`,
   `agent.permission.answer`, `agent.task.guide`). An external agent operating
   Last Answer through intents/deep-links is the reverse/operator direction;
   it follows the first direction, where Last Answer acts as ACP client and
   launches ACP agents against the shared document (ADR 0012).
...
- Runtime settings become **per-agent-binding data** on the doc roster
  (backend, model, profile, permission default) — no global agent config.
  Application-managed credentials stay device-local in secure storage;
  external ACP agents retain native authentication and no secret is copied
  into the shared document (ADR 0012).
...
- [ADR 0008](0008-local-first-model-federation.md) supersedes Decision 3's
  original statement that the meaning tree itself is device-local: the
  shared typed meaning/document graph is the CRDT world; host snapshots,
  derived cuts and runtime state remain device-local.
- [ADR 0012](0012-writable-agent-doc-and-acp-client-first.md) clarifies that
  the agent document is a writable shared work surface, not a transcript
  product. First, Last Answer acts as ACP client and launches ACP agents.
  Cross-device mesh and later VPN/Internet/VPS transport extend the same
  world; external ACP-client ingress to Last Answer follows only after the
  outbound path is proven. This supersedes the earlier wording in Decision 5
  that described Pi operating Last Answer through intents as the initial path.
