...
> **Historical operational handoff.** Its landed-model notes may remain useful,
> but its unfinished checklist and old validation claims are not the current
> forward plan. Use [PLAN.md](PLAN.md) for active work and
> [ADR 0012](decisions/0012-writable-agent-doc-and-acp-client-first.md) for
> the shared-document and ACP-client-first direction.
...
- MCP (`lib/mcp/doc_mcp_tools.dart` + `lib/main.dart`): `doc_state`, `doc_discuss_block`, `doc_collapse` (with `rewrite` param); verified live on macOS (`ws://127.0.0.1:<port>/<session>/ws`).
...
Use the running macOS debug app (already connected at `ws://127.0.0.1:<port>/<session>/ws`) or start a new one (`fvm flutter run -d macos --debug`). Then run this exact sequence via `runClientTool` (batch them — don't do one at a time):
