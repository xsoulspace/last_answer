...
> Investigation brief: local mesh-remote-agent-analysis brief.
> Scope: `dart_flutter_packages/pkgs/universal_storage_{mesh,mesh_transport,convergence,sync}`
> (found in the sibling `dart_flutter_packages` checkout), last_answer app wiring,
> and the ADR corpus.
> All paths below are relative to the repo root unless prefixed otherwise.
>
> **Scope note (ADR 0012):** this is an implementation investigation, not the
> current product contract. Its proposals to stream into a chat document or
> describe the host as the only writer to the world predate the writable shared
> document direction. New work must treat the shared document/meaning graph as
> canonical, conversations as projections, and the workspace owner as the sole
> code-filesystem materializer/oracle. The outbound Last Answer ACP-client path
> is first; remote collaboration and external operator ingress have separate,
> later gates in [PLAN.md](../PLAN.md).
...
op on the shared replica. ADR 0005 §3 names the topology ("agents execute
on the device that owns the workspace; every other peer holds input rights:
text edits, task claims, permission answers, proposals — delivered as ops").
An earlier `agents-in-docs.md` Phase 5 draft also said task assignment and
permission answers ride the existing round-trip; that draft has since been
replaced by the ADR 0012 sequence. The missing piece identified by this
historical investigation was the op schema + host-side fold observer that
turns a `task/<turnId>` op into `HarnessSessionController.delegate` (see
the design below); it is not a current release-order requirement.
...
- Host-side fold observer: extend the existing sync hook —
  ...
  The owner host alone materializes code changes and runs the filesystem
  oracle; authorized participants may contribute attributed operations to the
  shared document graph (ADRs 0008 and 0012).
