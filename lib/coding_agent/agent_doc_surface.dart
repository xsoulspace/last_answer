...
final class AgentDocSurface extends StatefulWidget {
  ...
  final ProviderConfigurationStore? providerConfiguration;
...
class _AgentDocSurfaceState extends State<AgentDocSurface>
  ...
  late final ProviderConfigurationStore _providerConfiguration =
      widget.providerConfiguration ?? ProviderConfigurationStore();
  ...
  bool _hostedConsentGranted = false;
  ...
  void initState() {
    ...
    unawaited(() async {
      if (await _syncConfigBeforeTurn()) await _controller.ensureStarted();
    }());
  ...
  HarnessHostConfig _configFor(final ProjectModelDoc doc) {
    ...
    return base.copyWith(
      ...
      model: agent.backend == 'jev' ? 'jev-1.13' : base.model,
  ...
  AgentDocDebugState _projectState() {
    ...
    return AgentDocDebugState(
      ...
      runtimeProfile: controller.config.meaningProfile ? 'meaning' : 'commands',
      sessionId: current?.id,
  ...
  ({bool ok, String message}) bindFromIntent({
    ...
    setState(() {});
    unawaited(_syncConfigBeforeTurn().catchError((final _) => false));
    return (
      ok: true,
      message: check == null
  ...
  ({bool ok, String message}) guideFromIntent(final String guidance) {
    ...
    setState(() {});
    unawaited(_delegate());
    return (ok: true, message: 'guidance recorded; continuing with guidance.');
  }
  ...
  Future<bool> _syncConfigBeforeTurn() async {
    if (_controller.isRunning) return false;
    final agent = _doc.agent;
    var apiKey = _controller.config.apiKey;
    final backend = agent?.backend ?? _controller.config.backend;
    if (backend == 'open_router' || backend == 'jev') {
      final workspace = _workspaceField.text.trim();
      final config = await _providerConfiguration.resolveOpenRouter(
        documentId: _doc.id.value,
        model: _controller.config.model,
        workspace: workspace,
      );
      switch (config.readiness) {
        case HostedDispatchReadiness.ready:
          apiKey = config.apiKey;
        case HostedDispatchReadiness.missingCredential:
          _controller.error =
              'OpenRouter credential is missing on this device.';
          if (mounted) setState(() {});
          return false;
        case HostedDispatchReadiness.consentRequired:
          _controller.error =
              'Confirm cloud use for this document, model, and workspace '
              'before sending.';
          if (mounted) setState(() {});
          return false;
      }
    ...
    await _controller.switchBackend(
      _controller.config.copyWith(
        ...
        jevBinding: backend == 'jev' && apiKey != null
            ? _controller.config.backend == 'jev' &&
                      _controller.config.apiKey == apiKey &&
                      _controller.config.jevBinding != null
                  ? _controller.config.jevBinding
                  : jevDecisionBinding(apiKey: apiKey)
            : null,
        clearJevBinding: backend != 'jev',
  ...
  Future<void> _switchBackend(final String backend) async {
    ...
    final entered = _keyField.text.trim();
    if ((backend == 'open_router' || backend == 'jev') && entered.isNotEmpty) {
      await _providerConfiguration.replaceOpenRouterCredential(entered);
      _keyField.clear();
    }
    await _controller.switchBackend(
      _controller.config.copyWith(
  ...
  Future<void> _grantHostedConsent() async {
    final workspace = _workspaceField.text.trim();
    if (workspace.isEmpty) {
      setState(() => _routingError = 'Bind a workspace before consenting.');
      return;
    }
    await _providerConfiguration.grantHostedConsent(
      documentId: _doc.id.value,
      provider: 'open_router',
      model: _controller.config.model,
      workspace: workspace,
    );
    setState(() => _hostedConsentGranted = true);
  }
            ...
            children: [
              ...
              if (_showSetup)
                _SetupPane(
                  ...
                  hostedConsentGranted: _hostedConsentGranted,
                  onHostedConsent: _grantHostedConsent,
                ),
              Expanded(
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.stretch,
...
class _SetupPane extends StatelessWidget {
  const _SetupPane({
    ...
    required this.hostedConsentGranted,
    required this.onHostedConsent,
  });
  ...
  final bool hostedConsentGranted;
  final Future<void> Function() onHostedConsent;
      ...
      child: Column(
        ...
        children: [
          ...
          if (backend == 'open_router' || backend == 'jev')
            Row(
              children: [
                SizedBox(width: 84, child: Text('CLOUD', style: _label(theme))),
                Expanded(
                  child: Text(
                    hostedConsentGranted
                        ? 'consent recorded for this document/workspace'
                        : 'confirm provider/model/workspace before sending',
                    style: _mono(theme),
                  ),
                ),
                _TextToggle(
                  label: hostedConsentGranted ? '[confirmed]' : '[confirm]',
                  active: hostedConsentGranted,
                  onTap: busy ? () {} : () => unawaited(onHostedConsent()),
                  toggleKey: const Key('coding_agent.hosted_consent'),
                ),
              ],
            ),
          Row(
              ...
              Expanded(
                child: editing
                    ? CallbackShortcuts(
                        ...
                        child: TextField(
                          key: Key('coding_agent.queue.edit.field.${step.id}'),
        ...
        children: [
          SizedBox(
            ...
            child: Text(
              ...
              style: _label(theme).copyWith(color: theme.colorScheme.onSurface),
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
            ),
          ),
          ...
          Row(
            children: [
              ...
              _TextToggle(
                ...
                onTap: () =>
                    widget.onSetRouting(!controller.remotePermissionRouting),
...
final class AgentDocShadow {
  ...
  Future<({bool ok, String message})> answerRemotePermission({
    ...
    if (pending == null) {
      return (ok: false, message: 'no pending remote permission request.');
    }
    try {
