...
## 2026-09-22 — consumer plan rebaseline (documentation only)

- Replaced the contradictory Phase/R9 execution ordering with C0–C6 for
  the existing macOS single-workspace agent-document coding workflow.
  Retained outstanding Phase 2–6, surface/profiler/trace and console-migration
  obligations explicitly as deferred work. This is planning, not their
  implementation or a readiness claim.
- Moved completed-status instructions out of the active plan: the historical
  Phase 1.5 human gate was recorded green on 2026-09-05 in harness
  `benchmark/runs/delegation_phase1_5.md` (initial honest FAIL, guided PASS,
  fixture oracle and restoration); R9.a's intent loop was recorded in
  `benchmark/runs/delegation_r9.md` (1,552 tokens, 43.6 s, one PASS row plus
  an off-task-write rejection row). Those measurements remain historical.
- Existing source confirms workspace/check/backend callback wiring in
  `lib/home/project_view.dart`; C0 must revalidate it. The old instruction
  to fix that already-present callback is retired; C2 separately addresses
  missing completed-turn durability. Source also contains queue persistence
  and registry machinery, so their remaining acceptance is not a greenfield
  implementation task.
- Historical Phase 5 fake/local gates and most real two-device steps were
  already recorded; the unresolved peer-permission completion, soak,
  snapshot adoption and later co-editing gates remain in PLAN.md.
- Current evidence limitation: the attempted focused command
  `fvm flutter test --no-pub test/coding_agent/agent_doc_persistence_test.dart test/coding_agent/harness_host_test.dart`
  exited 69 before tests ran because the local Xcode license is unaccepted.
  No runtime, passing-test, provider-capability or release claim is added.


## 2026-09-22 — shared-world scope correction (documentation only)

- Corrected C0–C6 release wording: one local device/user/workspace and one
  foreground task constrain the initial workload, not the number or kinds of
  actors. Existing ADRs 0006–0008 already distinguish world, projections,
  actors and reusable model resources; central M0 is now required by C6.
- Added origin-bound typed events, durable provenance/privacy, permissions,
  targeted cancellation and shared-world restoration acceptance. The local
  gate exercises human/mechanical actors and two model-backed actors sharing
  a model resource; advanced squads and distribution remain deferred.
- Optional decision providers may reason/code through authorized semantic
  choices. Jev is an example, disabled by default. J1-T/R/C/M evidence and
  promotion remain separate; R/C experiments do not wait for consumer release
  after their generic safety gates. No implementation or runtime claim.

## 2026-09-24 — writable agent document and ACP-client-first rebaseline (documentation only)

- Added [ADR 0012](decisions/0012-writable-agent-doc-and-acp-client-first.md):
  the writable shared document/meaning graph is the product; typed execution
  events are task facts; conversations and activity views are projections.
  Last Answer first launches ACP agents as the ACP client. External ACP-client,
  CLI, MCP or IntentCall ingress follows only after that path is proven.
- Rebased [PLAN.md](PLAN.md) C0–C6 around the local human + ACP-agent shared
  document/workspace slice, durable attributed edits/threads/diffs, and
  owner-side materialization and verification. Remote mesh collaboration is
  staged next; VPN/Internet/VPS transport remains later work.
- Aligned the North Star, design law, agent-doc phases, document pillars,
  conversation projection, multiplayer gate runbook, older ADR/report
  clarifications, handoffs and navigation. ACP file operations must cross a
  revision-bound proposal boundary; only the owner materializes and verifies
  code changes. ADR 0008's shared meaning graph, single materializer and
  remote-transport roadmap remain the architectural substrate.
- Documentation only: no feature, ACP interoperability, mesh completion,
  provider readiness, remote access, or release acceptance is claimed. The
