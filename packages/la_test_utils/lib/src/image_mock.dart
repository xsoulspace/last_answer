...
Future<void> ensureImagesPreloaded(final WidgetTester tester) async {
...
R mockNetworkImagesFor<R>(final R Function() body) {
  return HttpOverrides.runZoned(
    body,
    createHttpClient: (final _) => createMockImageHttpClient(),
  );
}
...
MockHttpClient createMockImageHttpClient() {
  ...
  when(client.getUrl(any))
      .thenAnswer((final _) => Future<HttpClientRequest>.value(request));
  when(request.headers).thenReturn(headers);
  when(request.close())
      .thenAnswer((final _) => Future<HttpClientResponse>.value(response));
