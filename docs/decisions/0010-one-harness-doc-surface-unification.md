...
- Status: **Proposed** (ADR 0012 clarifies the ACP-client-first direction
  for question 1; the remaining gates and design are still proposed)
- Date: 2026-09-08
- North Star impact: `strengthens` — value path 3 (document-first,
...
Per [ADR 0012](0012-writable-agent-doc-and-acp-client-first.md), the first
outbound ACP-client binding is part of the local writable-document slice;
it is not postponed until after an embedded-only release.
...
1. **RESOLVED (2026-09-24, ADR 0012):** Last Answer is the ACP client and
   launches external ACP agents through the same agent-document runtime
   seam in the first writable-document slice. Do not create a parallel
   chat surface or wait for an embedded-only release before beginning
   ACP-agent interoperability. The shared document/meaning graph remains
   canonical; conversation is a projection.
